...
def piecewise_normalize( score,  left,  right,
		 threshold,  new_left,
...
def get_spy_similar(v1, v2):
    ...
    final_score = piecewise_normalize(score_full, 0.0, 100.0, threshold, 0, 100, 80)
    print('spy ', num, score_full, score_full, final_score)
    return score_full


def compare_file(file_path_0, file_path_1, score_limit):
    print('compare ', os.path.abspath(file_path_0), os.path.abspath(file_path_1))
...
if __name__ == "__main__":
    parser = ArgumentParser(description="compare tinynn and model binary outputs")
    parser.add_argument("file_or_dir", nargs=2, type=str)
    parser.add_argument("--score", type=float, required=False, default=50)
    args = parser.parse_args()
    score = args.score
    compare_file_cnt = 0
    if os.path.isdir(args.file_or_dir[0]) and os.path.isdir(args.file_or_dir[1]):
        ...
        assert os.path.isfile(args.file_or_dir[0]), 'can not find {}'.format(args.file_or_dir[0])
        assert os.path.isfile(args.file_or_dir[1]), 'can not find {}'.format(args.file_or_dir[1])
