/**
 * \file runtime/example/Nonstandard_OS/bare_board/bare_board_qemu_arm_example.c
 *
...
void run_tinynn_test(unsigned char* model_ptr, int model_len,
                     unsigned char* inptr, int input_len,
    ...
    EXAMPLE_CHECK(LITE_load_model_from_mem(model, model_ptr, model_len),
                  "load model error. \n");

    LiteTensor input;
    EXAMPLE_CHECK(LITE_get_io_tensor(model, input_name, LITE_INPUT, &input),
                  "get input tensor failed\n");
    EXAMPLE_CHECK(LITE_reset_tensor_memory(input, inptr, input_len),
...
int main() {
    ...
    run_tinynn_test(model_tiny, model_tiny_len, input_bin, input_bin_len,
                    "data", print_uart0_ftm);
