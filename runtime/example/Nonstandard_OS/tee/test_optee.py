...
def main():
    ...
    parser.add_argument(
        "--optee_repo_dir",
        help="optee repo dir, please refs https://optee.readthedocs.io/en/latest/building/gits/build.html to init",
        required=True,
    )
    ...
    for s in optee_repo_sub:
        ...
        assert os.path.isdir(
            s_dir
        ), "invalid --optee_repo_dir : can not find dirs: {}".format(s_dir)
    ...
    assert os.path.isdir(
        example_git_dir
    ), "invalid optee repo, can not find: {}".format(example_git_dir)
    logging.debug("force clean old example_dir")
    subprocess.check_call(
        "cd {} && git clean -xdf && git reset --hard".format(example_dir), shell=True
    )

    # prepare new test example
    subprocess.check_call(
        "cp -rf tee {}/megcc_inference".format(example_dir), shell=True
    )
    subprocess.check_call(
    ...
    assert os.path.isdir(build_git_dir), "invalid optee repo, can not find: {}".format(
        build_git_dir
    )
    logging.debug("force clean old build_dir")
    subprocess.check_call(
        "cd {} && git clean -xdf && git reset --hard".format(build_dir), shell=True
    )
    # patch details:
    ...
    subprocess.check_call(
        "cp tee/{} {} && cd {} && git apply {}".format(
            patch, build_dir, build_dir, patch
        ),
        shell=True,
    )
    ...
    subprocess.check_call(
        "cd {} && make -f qemu_v8.mk all -j$(nproc)".format(build_dir), shell=True
    )
    ...
    logging.debug(
        "run megcc_inference example by cmd: {} timeout: {}".format(cmd, max_time)
    )
    try:
        subprocess.check_call(cmd, shell=True, timeout=max_time)
        raw_log = subprocess.check_output(
            "cd {} && cat raw_log.log".format(build_dir), shell=True
        ).decode("utf-8")
        ca_key_logs = ["invoke init model cmd,seconds", "run megcc tee example success"]
