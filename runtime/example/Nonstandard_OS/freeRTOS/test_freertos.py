...
def main():
    ...
    parser.add_argument(
        "--free_rtos_repo_dir",
        help=
        "freeRTOS repo dir, please download from https://github.com/FreeRTOS/FreeRTOS",
        required=True,
    )
    ...
    subprocess.check_call(
        "cd {} && git clean -xdf && git reset --hard".format(
            args.free_rtos_repo_dir),
        shell=True,
    )

    # prepare new test example
    minimal_dir = os.path.join(args.free_rtos_repo_dir,
                               "FreeRTOS/Demo/Common/Minimal")
    subprocess.check_call(
        "cp -rf freeRTOS/tinynn_freeRTOS_example.c {}/".format(minimal_dir),
        shell=True)
    subprocess.check_call(
        "cp -rf {} {}/tinynn_sdk".format(args.tinynn_lib_install_dir,
                                         minimal_dir),
        shell=True,
    )
    ...
    subprocess.check_call(
        "cp freeRTOS/tinynn_example_test_at_minimal_rtos.patch {} && cd {} && git apply tinynn_example_test_at_minimal_rtos.patch"
        .format(args.free_rtos_repo_dir, args.free_rtos_repo_dir),
        shell=True,
    )

    # build freeRTOS repo
    logging.debug("build freeRTOS repo")
    iar_dir = os.path.join(args.free_rtos_repo_dir,
                           "FreeRTOS/Demo/CORTEX_MPS2_QEMU_IAR_GCC/build/gcc")
    subprocess.check_call("cd {} && make -j$(nproc)".format(iar_dir),
                          shell=True)
    ...
    def sleep_and_kill_thread():
        ...
        cnt = 0
        while True:
            logging.debug("sleep 1s cnt: {} vs max_time: {}".format(
                cnt, max_time))
            cnt = cnt + 1
            time.sleep(1)
            if cnt == max_time:
    ...
    cmd = "cd {} && rm -rf raw_log.log && qemu-system-arm -semihosting -machine mps2-an385 -cpu cortex-m3 -m 16M -kernel output/RTOSDemo.out -monitor none -nographic -serial stdio | tee raw_log.log || true".format(
        iar_dir)
    logging.debug("run tinynn_inference example by cmd: {} timeout: {}".format(
        cmd, max_time))
    try:
        subprocess.check_call(cmd, shell=True, timeout=max_time + 5)
        raw_log = subprocess.check_output(
            "cd {} && cat raw_log.log".format(iar_dir),
            shell=True).decode("utf-8")
        raw_log = "head" + raw_log
        key_logs = [
            "start tinynn inference!!!", "run tinynn FreeRTOS example success"
        ]
        for log in key_logs:
