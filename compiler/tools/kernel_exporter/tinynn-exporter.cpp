...
int main(int argc, char** argv) {
    auto k_name_desc =
            "input kernel name, valid option:\n" + KernelExporter::support_kernels();
    cl::opt<std::string> KernelName("kernel", cl::Required, cl::desc(k_name_desc));
    auto arch_desc =
            "the platform arch, valid options:\n" + KernelExporter::support_archs();
    ...
    cl::opt<bool> print_to_console(
            "print_to_console", cl::desc("Print kernel body to console"));
    cl::opt<bool> Verbose(
    ...
    KernelExporter exporter(
            KernelName.getValue(), kernelArch.getValue(), use_default_attr.getValue(),
