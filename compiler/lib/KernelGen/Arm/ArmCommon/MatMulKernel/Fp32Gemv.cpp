/**
 * \file
 * compiler/lib/KernelGen/Arm/ArmCommon/MatMulKernel/Fp32Gemv.cpp
 *
...
#define INIT(i) float32x4_t sum##i = vdupq_n_f32(0);
#define LOADA(i) float32x4_t a##i = vld1q_f32(A + (m + i) * Astride + k);
#define CALADD(i) sum##i = vmlaq_f32(sum##i, a##i, b0);
#define CALOTHER(i) acc[i] += A[(m + i) * Astride + k] * D[n * Bstride + k];
#define STORE(i) C[(m + i) * Cstride + n] = acc[i] + vaddvq_f32(sum##i);
...
#define EXE_BLOCK(i)                                                           \
  ...
  for (; k + 4 <= K; k += 4) {                                                 \
    UNROLL_FUNC(LOADA, i);                                                     \
    float32x4_t b0 = vld1q_f32(D + n * K + k);                                 \
