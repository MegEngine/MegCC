...
namespace {
std::string prefetch_4x() {
    std::string prefetch_macro = "";
    prefetch_macro = R"(
    ...
    return prefetch_macro + KernelGen::ArmCommon::gen_common_prefetch_4x_f32();
}
...
#define LOADB(i) float32x4_t b##i = vld1q_f32(B + k * Bstride + n+4*i);
#define LOADC(i) float32x4_t c##i = vld1q_f32(C + m * Cstride + n+4*i);
#define CALADD(i) c##i = vmlaq_f32(c##i,a0,b##i);
#define STORE(i) vst1q_f32(C + m * Cstride + n + 4*i,c##i);
#define EXE_BLOCK(i)\
UNROLL_FUNC(LOADB,i)\
UNROLL_FUNC(LOADC,i)\
...
UNROLL_FUNC(STORE,i)
    ...
    for (size_t m = 0; m < M; m++) {
      for (size_t k = 0; k < K; k++) {
        size_t n = 0;
        float32x4_t a0 = vdupq_n_f32(A[m * Astride + k]);
        for(;n+64<=N;n+=64){
            prefetch_4x(B + k * Bstride + n);
            prefetch_4x(C + m * Cstride + n);
            EXE_BLOCK(16)
        }
        if(n+32<=N)
        {
