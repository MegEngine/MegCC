/**
 * \file
 * compiler/lib/KernelGen/Arm/ArmCommon/ElemwiseHelper/BinaryHelper.cpp
 *
...
std::string BinaryCode<VEC_VEC>() {
    std::string body = R"(
        ...
        for(; index + 7 < nr_elem; index += 8) {
            ...
            float32x4_t vsrc1_0 = vld1q_f32(src1);
            float32x4_t vsrc1_1 = vld1q_f32(src1 + 4);
...
std::string BinaryCode<VEC_SCALAR>() {
    std::string body = R"(
        ...
        const float * src1 = ${source1};
        float * dst = ${dst};
        float32x4_t vsrc1_0 = vdupq_n_f32(src1[0]);
        size_t index = 0;
        for(; index + 7 < nr_elem; index += 8) {
            float32x4_t vsrc0_0 = vld1q_f32(src0);
            float32x4_t vsrc0_1 = vld1q_f32(src0 + 4);
    ...
    std::string body = R"(
        ...
        for(size_t c=0; c<channel; c++){
            ...
            for(; index + 7 < channel_stride; index += 8){
                float32x4_t vsrc0_0 = vld1q_f32(src0_ptr + index);
                float32x4_t vsrc0_1 = vld1q_f32(src0_ptr + index + 4);
                float32x4_t vsrc1_0 = vld1q_f32(src1_ptr + index);
                float32x4_t vsrc1_1 = vld1q_f32(src1_ptr + index + 4);
                ${kernel_simd_unroll(2, dst, vsrc0_0, vsrc1_0, vsrc0_1, vsrc1_1)}
                dst += 8;
            }
...
std::string BinaryCode<VEC_BCAST101>() {
    std::string body = R"(
        ...
        for(size_t b=0; b<batch; b++){
            for(size_t c=0; c<channel; c++){
                float32x4_t vsrc1_0 = vdupq_n_f32(src1[c]);
                size_t index = 0;
...
std::string BinaryCode<VEC_BCAST101x4>() {
    std::string body = R"(
        ...
        for(size_t b=0; b<batch; b++){
            for(size_t c=0; c<channel; c++){
                float32x4_t vsrc1_0 = vld1q_f32(src1 + c * 4);
                size_t index = 0;
                for(; index + 7 < nr_elem_per_channel; index += 8) {
                    float32x4_t vsrc0_0 = vld1q_f32(src0);
                    float32x4_t vsrc0_1 = vld1q_f32(src0 + 4);
    ...
    std::string body = R"(
        ...
        if (nr_elem == nr_elem_in0 && nr_elem_in0 == nr_elem_in1){
            ...
            if(nr_elem_in0 > nr_elem_in1){
                ...
                const float * src1 = inputs[1]->ptr;
                float * dst = outputs[0]->ptr;
                float32x4_t vsrc0_0 = vdupq_n_f32(src0[0]);
                size_t index = 0;
                for(; index + 7 < nr_elem; index += 8) {
                    float32x4_t vsrc1_0 = vld1q_f32(src1);
                    float32x4_t vsrc1_1 = vld1q_f32(src1 + 4);
