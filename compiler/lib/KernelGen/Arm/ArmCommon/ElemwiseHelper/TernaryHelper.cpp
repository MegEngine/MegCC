/**
 * \file
 * compiler/lib/KernelGen/Arm/ArmCommon/ElemwiseHelper/TernaryHelper.cpp
 *
...
std::string TernaryCode<VEC_VEC_VEC>() {
    std::string body = R"(
        ...
        for(; index + 7 < nr_elem; index += 8) {
            ...
            float32x4_t vsrc2_0 = vld1q_f32(src2);
            float32x4_t vsrc2_1 = vld1q_f32(src2 + 4);
...
std::string ElemwiseGenTernaryFuseMulAdd3::GenKernelSimdUnroll(
    ...
    for (int i = 0; i < unroll; i++) {
        writer << "\n float32x4_t tmp" << i << " = vfmaq_f32(" << strs[str_id + 2]
               << "," << strs[str_id + 0] << "," << strs[str_id + 1] << ");";
        writer << "\n vst1q_f32((" << dst << " + 4 * " << i << "), tmp" << i << ");";
        str_id += 3;
    }
    return writer.str();
}
