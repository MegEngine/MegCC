/**
 * \file
 * compiler/lib/KernelGen/Arm/ArmCommon/ElemwiseHelper/ElemwiseHelper.h
 *
...
public:
    ...
    bool m_inline_mode;
    std::unique_ptr<ArmSimdHelper> m_src_simd;
    std::unique_ptr<ArmSimdHelper> m_dst_simd;
    bool m_i32_to_qs8;
    ElemwiseGenUnary(std::string src_dtype = "f32",
        ...
        m_src_simd = std::make_unique<ArmSimdHelper>(src_dtype);
        m_dst_simd = std::make_unique<ArmSimdHelper>(dst_dtype);
...
DEFINE_NNARY_OP(ElemwiseGenUnaryRelu)
...
DEFINE_BINARY_OP(ElemwiseGenBinaryFuseAddSigmoid)
