...
#undef RSCALE
        ...
        static inline void calc_cache_8uc3_1(const uint8_t* src, const int src_h_stride, const uint8_t* dst, const int OW,
                       ...
                       const int* tabsy,
                       const float* tabrx,
                       const float* tabry, int dx,
                       float* cache0, float* cache1) {
            (void)tabrx;
            const uint8_t* psrc1 = src + (tabsx[dx] + 1) * src_h_stride;
        ...
        void ${kernel_sig}{
            ...
            if (C == 1 && use_cache) {
                alignas(16) int tabsx[OH];
                alignas(16) int tabsy[OW];
                alignas(16) float tabrx[OH];
                alignas(16) float tabry[OW];
                build_tabs_linear_8u(IH, IW, OH, OW, tabsx, tabsy, tabrx, tabry);


                int dstrows = OH;
                int dstcols = OW;
                alignas(16) float cache0_body[dstcols];
                alignas(16) float cache1_body[dstcols];
                float* cache0 = &cache0_body[0];
                float* cache1 = &cache1_body[0];
                for (int dx = 0; dx < dstrows; ++dx) {
                    if (dx == 0 || tabsx[dx] != tabsx[dx - 1]) {
                        if (dx > 0 && tabsx[dx] == tabsx[dx - 1] + 1) {
                            float* temp = cache0;
                    ...
                    const float* cache0_ptr = cache0;
                    const float* cache1_ptr = cache1;
                    float32x4_t v_rx = vdupq_n_f32(rx);
                    float32x4_t v_irx = vdupq_n_f32(irx);
                    for (; dy + 16 <= dstcols; dy += 16) {
                        float32x4_t v_cache0_0;
                        float32x4_t v_cache1_0;
                        float32x4_t v_cache0_4;
                        float32x4_t v_cache1_4;
                        float32x4_t v_cache0_8;
                        float32x4_t v_cache1_8;
                        float32x4_t v_cache0_c;
                        float32x4_t v_cache1_c;

                        v_cache0_0 = vld1q_f32(cache0_ptr + dy + 0x0);
                        v_cache1_0 = vld1q_f32(cache1_ptr + dy + 0x0);
                        v_cache0_4 = vld1q_f32(cache0_ptr + dy + 0x4);
                        v_cache1_4 = vld1q_f32(cache1_ptr + dy + 0x4);
                        v_cache0_8 = vld1q_f32(cache0_ptr + dy + 0x8);
                        v_cache1_8 = vld1q_f32(cache1_ptr + dy + 0x8);
                        v_cache0_c = vld1q_f32(cache0_ptr + dy + 0xc);
                        v_cache1_c = vld1q_f32(cache1_ptr + dy + 0xc);

                        float32x4_t v_ans0, v_ans4, v_ans8, v_ansc;
                        v_ans0 = vmlaq_f32(vmulq_f32(v_rx, v_cache1_0),
                                                        v_irx, v_cache0_0);
                        v_ans4 = vmlaq_f32(vmulq_f32(v_rx, v_cache1_4),
                                                        v_irx, v_cache0_4);
                        v_ans8 = vmlaq_f32(vmulq_f32(v_rx, v_cache1_8),
                                                        v_irx, v_cache0_8);
                        v_ansc = vmlaq_f32(vmulq_f32(v_rx, v_cache1_c),
                                                        v_irx, v_cache0_c);

                        uint16x8_t v_half16_0, v_half16_1;
                        v_half16_0 = vcombine_u16(vqmovn_u32(vreinterpretq_u32_s32(vcvtaq_s32_f32(v_ans0))), vqmovn_u32(vreinterpretq_u32_s32(vcvtaq_s32_f32(v_ans4))));  // x0
                        v_half16_1 = vcombine_u16(vqmovn_u32(vreinterpretq_u32_s32(vcvtaq_s32_f32(v_ans8))), vqmovn_u32(vreinterpretq_u32_s32(vcvtaq_s32_f32(v_ansc))));  // y0

                        vst1q_u8(pdst + dy, vcombine_u8(vqmovn_u16(v_half16_0), vqmovn_u16(v_half16_1)));
                    }
                    for (; dy < dstcols; ++dy) {
                        uint8_t* pcdst = pdst + dy;
                        pcdst[0] =
                                output_converter(rx * cache1[dy] + irx * cache0[dy]);
                    }
                }
            } else if (C == 3 && use_cache) {
                alignas(16) int tabsx[OH];
                ...
                for (int dx = 0; dx < dstrows; ++dx) {
                    ...
                    for (; dy < dstcols; dy += 3) {
                        uint8_t* pcdst = pdst + dy;
                        pcdst[0] = output_converter(rx * cache1[dy + 0] + irx * cache0[dy + 0]);
                        pcdst[1] = output_converter(rx * cache1[dy + 1] + irx * cache0[dy + 1]);
                        pcdst[2] = output_converter(rx * cache1[dy + 2] + irx * cache0[dy + 2]);
                    }
                }
