/**
 * \file
 * compiler/lib/KernelGen/Arm/ArmCommon/ConvKernel/Fp32/Fp32ConvNchwNchw44.cpp
 *
...
std::string ConvFloatNCHWNCHW44::GetKernelSymbol(TContext* ctx) const {
    ...
    return StringTemplate::StringTemplateArgs(ctx)
            .add("base_kernel_sym", ArmCommonConvImpl::GetKernelSymbol(ctx))
            .add("oc", oc)
            .add("ic", ic)
            .render(name_temp);
}
...
std::string render_kernel(TContext* ctx) {
    ...
    float* src_ptr, const float* filter_ptr, const float* bias_ptr,
            ...
            rep(step, dst_nr_ow){
                ${activate(c[1][step], dst_ptr + ld_dst_oc + step * simd_len)};
            }
        }
    )";
