/**
 * \file
 * compiler/lib/KernelGen/Arm/Arm64/Activation.h
 *
...
struct ActivationGenAsmBase {
    virtual std::string GenAsmFloat(std::vector<std::string> regs,
                                    std::vector<std::string> help_regs) = 0;
    virtual std::string GenAsmInt8(std::vector<std::string> regs,
                                   std::vector<std::string> help_regs) = 0;
    virtual std::string GenAsmFloat(std::vector<std::string> regs) {
...
public:
    std::string GenAsmFloat(std::vector<std::string>,
                            std::vector<std::string> help_regs) override {
        return "";
    }
    std::string GenAsmInt8(std::vector<std::string>,
                           std::vector<std::string> help_regs) override {
        return "";
    }
};
...
template <>
struct ActivationGenAsm<ArmCommon::NonlineMode::RELU>
        : public ActivationGenAsmBase {
public:
    //! the first register is the zero register
    std::string GenAsmFloat(std::vector<std::string> registers,
                            std::vector<std::string> help_regs) override {
        std::string r_zero = help_regs[0] + ".4s \\n\"\n";
        std::stringstream writer;
        writer << "\n";
        for (size_t i = 0; i < registers.size(); i++) {
            writer << "\"fmax " << registers[i] << ".4s, " << registers[i]
                   << ".4s, " << r_zero;
        }
        return writer.str();
    }
    std::string GenAsmInt8(std::vector<std::string> registers,
                           std::vector<std::string> help_regs) override {
