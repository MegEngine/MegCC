...
std::string Int8Conv1x1NCHW44::GetKernelSymbol(TContext* ctx) const {
    return "Armv7_int8_Conv1x1_" + ConvImpl::GetKernelSymbol(ctx);
}
...
std::string Int8Conv1x1NCHW44::GetInitBody(TContext* context) const {
...
std::string Int8Conv1x1NCHW44::GetKernelBody(TContext* context) const {
    std::stringstream writer;
    writer << R"(
#include <marm_neon.h>
#include <string.h>
