/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/MatMulKernel/Fp16Gemv.cpp
 *
...
bool Fp16GemvKernel::IsAvailable(TContext* context) const {
...
//! kernel gen
std::string Fp16GemvKernel::GetKernelSymbol(TContext* context) const {
    std::stringstream ss;
    ss << "GI_kernel_gemv_fp16_";
...
std::string Fp16GemvKernel::GetWorkspaceBody(TContext* context) const {
    std::stringstream ss;
    ...
    std::string workspace_temp = R"({
        ...
        if(ic == 1)*workspace = 0;
        else *workspace = in_layout.dims[0] * ic * 2;
...
std::string Fp16GemvKernel::GetKernelBody(TContext* context) const {
    ...
    // clang-format off
    writer << R"(
    gi_float16_t* A = (gi_float16_t*)inputs[0]->ptr;
    gi_float16_t* B = (gi_float16_t*)inputs[1]->ptr;
...
#define INIT(i) GI_FLOAT16_t sum##i = GiBroadcastFloat16(0);
#define LOADA(i) GI_FLOAT16_t a##i = GiLoadFloat16(A + (m + i) * Astride + k);
#define CALADD(i) sum##i = GiMlaqFloat16(sum##i, a##i, b0);
#define CALOTHER(i) acc[i] += A[(m + i) * Astride + k] * D[n * Bstride + k];
#define CUMULATE(i) \
acc[i] += GiReduceAddFloat16(sum##i);

#define STORE(i) C[(m + i) * Cstride + n] = acc[i];

#define EXE_BLOCK(i)                                                           \
  gi_float16_t acc[i] = {0};                                                          \
  UNROLL_FUNC(INIT, i);                                                        \
  size_t k = 0;                                                                \
  for (; k + 8 <= K; k += 8) {                                                 \
    UNROLL_FUNC(LOADA, i);                                                     \
    GI_FLOAT16_t b0 = GiLoadFloat16(D + n * K + k);                                 \
