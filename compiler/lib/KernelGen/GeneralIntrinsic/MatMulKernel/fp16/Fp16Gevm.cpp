/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/MatMulKernel/Fp16Gevm.cpp
 *
...
#define LOADB(i) GI_FLOAT16_t b##i = GiLoadFloat16(B + k * Bstride + n+8*i);
#define LOADC(i) GI_FLOAT16_t c##i = GiLoadFloat16(C + m * Cstride + n+8*i);
#define CALADD(i) c##i = GiMlaqFloat16(c##i,a0,b##i);
#define STORE(i) GiStoreFloat16(C + m * Cstride + n + 8*i,c##i);
#define EXE_BLOCK(i)\
UNROLL_FUNC(LOADB,i)\
UNROLL_FUNC(LOADC,i)\
UNROLL_FUNC(CALADD,i)\
UNROLL_FUNC(STORE,i)
    memset(C, 0, sizeof(gi_float16_t) * M * N);
    for (size_t m = 0; m < M; m++) {
      for (size_t k = 0; k < K; k++) {
        size_t n = 0;
        GI_FLOAT16_t a0 = GiBroadcastFloat16(A[m * Astride + k]);
        for(;n+64<=N;n+=64){
            EXE_BLOCK(8)
        }
        if(n+32<=N)
...
bool Fp16GevmKernel::IsAvailable(TContext* context) const {
...
//! kernel gen
std::string Fp16GevmKernel::GetKernelSymbol(TContext* context) const {
    std::stringstream ss;
    ss << "GI_kernel_gevm_fp16_";
...
std::string Fp16GevmKernel::GetKernelBody(TContext* context) const {
