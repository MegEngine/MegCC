/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/ConvKernel/Fp32ConvNchwNchw44.cpp
 *
...
bool ConvFloatNCHWNCHW44::IsAvailable(TContext* ctx) const {
    bool param_value_ok =
...
std::string ConvFloatNCHWNCHW44::GetWorkspaceBody(TContext* context) const {
    std::stringstream ss;
    ss << R"(
...
std::string render_init(int c_idx, int nr_ow, bool with_bias) {
    ...
    for (int src_idx = 0; src_idx < nr_ow; ++src_idx) {
        if (with_bias) {
            ss << "c[" << c_idx << "][" << src_idx
               << "] = GiFloat32Type2FixLenType(GiLoadFloat32(bias_ptr + " << c_idx
               << " * 4));";
        } else {
            ss << "c[" << c_idx << "][" << src_idx
...
std::string render_core(
    ...
    if (remain_n) {
        ...
        rep(src_idx, read_src_reg_size){
            src[src_idx] = GiFloat32Type2FixLenType(GiLoadFloat32(src_ptr + ${fh_idx} * packed_iw + src_idx * ${simd_len}));
        })";
    } else {
    ...
    for (int fw_idx = 0; fw_idx < filter_size; ++fw_idx) {
        ...
        for (int i = 0; i < nr_ow; ++i) {
            fw_ss << "c[0][" << i
                  ...
                  << i << "]), GiFixLenType2GiFloat32Type(weight[0][" << weight_idx
                  << "]),  GiFixLenType2GiFloat32Type(src[(" << i << " * ${stride} + "
                  << src_idx << ") / 4]), " << (i * stride + src_idx) % 4 << "));";
            if (is_big_oc) {
                fw_ss << "c[1][" << i
                      << "] = "
                         "GiFloat32Type2FixLenType(GiSimdFmaLane("
                         "GiFixLenType2GiFloat32Type(c[1]["
                      << i << "]), GiFixLenType2GiFloat32Type(weight[1][" << weight_idx
                      << "]),  GiFixLenType2GiFloat32Type(src[(" << i
                      << " * ${stride} + " << src_idx << ") / 4]), "
                      << (i * stride + src_idx) % 4 << "));";
            }
        }
...
std::string render_kernel(TContext* ctx) {
    ...
    static inline void nchw_nchw44_s${stride}_${filter_size}x${filter_size}_kernel_${nr_ow}_oc${oc_step}(const float* src_ptr, const float* filter_ptr, const float* bias_ptr,
                                            float* dst_ptr, const int packed_iw, const int packed_ic_stride, const int ld_dst_oc){
            const int pad_h = ${pad_h};
            const int pad_w = ${pad_w};
            const int simd_len = ${simd_len};
            GI_FLOAT32_FIXLEN_t c[${oc_step}/4][${ow_step}];
            ${render_init(0)}
            ${render_init(1)}

            for (int ic_idx = 0; ic_idx < ${ic}; ++ic_idx) {
                GI_FLOAT32_FIXLEN_t src[${src_reg_size}];
                GI_FLOAT32_FIXLEN_t weight[${oc_step}/4][${filter_size}];                
                ${render_core()}
    ...
    static inline void
    nchw_nchw44_s${stride}_${filter_size}x${filter_size}_kernel_remain_oc${oc_step}(const
    float* src_ptr, const float* filter_ptr, const float* bias_ptr,
            ...
            ${activate_init()}
            rep(step, dst_nr_ow){
                ${activate(GiFixLenType2GiFloat32Type(c[0][step]), dst_ptr + step * simd_len)};
            }
            rep(step, dst_nr_ow){
                ${activate(GiFixLenType2GiFloat32Type(c[1][step]), dst_ptr + ld_dst_oc + step * simd_len)};
            }
        }
    )";
    ...
    static inline void
    nchw_nchw44_s${stride}_${filter_size}x${filter_size}_kernel_${nr_ow}_oc${oc_step}(const
    float* src_ptr, const float* filter_ptr, const float* bias_ptr,
                                        float* dst_ptr, const int packed_iw,
...
std::string ConvFloatNCHWNCHW44::GetKernelBody(TContext* ctx) const {
    std::stringstream writer;
    auto src_tensor = ctx->getAttrOprand("operand:0");
    uint32_t ic = src_tensor.shape[1];
    auto dst_tensor = ctx->getAttrOprand(
