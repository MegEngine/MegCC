/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/ConvKernel/Fp32ConvNchwNchw88.cpp
 *
...
bool ConvFloat16NCHWNCHW88::IsAvailable(TContext* ctx) const {
    bool param_value_ok =
...
std::string ConvFloat16NCHWNCHW88::GetKernelSymbol(TContext* ctx) const {
    if (ctx) {
        ...
        auto dst_tensor = ctx->getAttrOprand(
                "operand:" + std::to_string(ctx->getAttrInt("nr_operands") - 1));
        uint32_t oc = dst_tensor.shape[1] * 8;
        std::string name_temp = "${base_kernel_sym}_nchw_nchw88_fp16_oc${oc}_ic${ic}";
        ...
        return "GI_kernel_conv2d_nchw_nchw88_fp16";
    }
}

std::string ConvFloat16NCHWNCHW88::GetInitBody(TContext* ctx) const {
    ...
    const gi_float16_t* inptr = in_weights->ptr;
    for (int oc_idx = 0; oc_idx < ${oc}; oc_idx += oc_step) {
        const gi_float16_t* in_ptr_oc = inptr + oc_idx * filter_oc_stride;
        gi_float16_t* dst_ptr_oc = outptr + oc_idx * filter_oc_stride;
        for (int kh_idx = 0; kh_idx < ${kh}; ++kh_idx) {
            for (int kw_idx = 0; kw_idx < ${kw}; ++kw_idx) {
                for (int ic_idx = 0; ic_idx < ${ic}; ++ic_idx) {
                    GI_FLOAT16_t vsrc = GiLoadFloat16(in_ptr_oc);
                    GiStoreFloat16(dst_ptr_oc + ic_idx * filter_ic_stride, vsrc);
                    in_ptr_oc += oc_step;
                }
                dst_ptr_oc += oc_step;
            }
...
std::string ConvFloat16NCHWNCHW88::GetWorkspaceBody(TContext* context) const {
    std::stringstream ss;
    ss << R"(
    ...
    std::string workspace_temp = R"({
        ...
        *workspace = (size_t)ic * packed_ih * packed_iw * 2;
        return TinyNN_SUCCESS;
    })";
    ss << StringTemplate::StringTemplateArgs(context)
                    .add_ctx_int("pad_h")
                    .add_ctx_int("pad_w")
                    .add("cacheline_byte", 64)
...
std::string render_init(int c_idx, int nr_ow, bool with_bias) {
    ...
    for (int src_idx = 0; src_idx < nr_ow; ++src_idx) {
        if (with_bias) {
            ss << "c[" << c_idx << "][" << src_idx
               << "] = GiFloat16Type2FixLenType(GiLoadFloat16(bias_ptr + " << c_idx
               << " * 8));";
        } else {
            ss << "c[" << c_idx << "][" << src_idx
               << "] = GiFloat16Type2FixLenType(GiBroadcastFloat16(0.0));";
        }
    }
    return ss.str();
}
std::string render_core(
    ...
    if (remain_n) {
        ...
        rep(src_idx, read_src_reg_size){
            src[src_idx] = GiFloat16Type2FixLenType(GiLoadFloat16(src_ptr + ${fh_idx} * packed_iw + src_idx * ${simd_len}));
        })";
    } else {
        for (int src_idx = 0; src_idx < src_reg_size; ++src_idx) {
            fw_ss << "src[" << src_idx
                  << "] = GiFloat16Type2FixLenType(GiLoadFloat16(src_ptr + "
                     "${fh_idx} * packed_iw + "
                  << src_idx << "* ${simd_len}));\n";
        }
    }
    ...
    for (int fw_idx = 0; fw_idx < filter_size; ++fw_idx) {
        fw_ss << "weight[0][" << fw_idx
              << "] = GiFloat16Type2FixLenType(GiLoadFloat16(filter_ptr + "
                 "${fh_idx} * ${ld_weight_fw} + "
              << fw_idx << " * ${simd_len}));\n";
    }
    if (is_big_oc) {
    ...
    for (int fw_idx = 0; fw_idx < filter_size; ++fw_idx) {
        ...
        for (int i = 0; i < nr_ow; ++i) {
            fw_ss << "c[0][" << i
                  << "] = "
                     "GiFloat16Type2FixLenType(GiSimdFmaLaneFloat16("
                     "GiFixLenType2GiFloat16Type(c[0]["
                  << i << "]), GiFixLenType2GiFloat16Type(weight[0][" << weight_idx
                  << "]),  GiFixLenType2GiFloat16Type(src[(" << i << " * ${stride} + "
                  << src_idx << ") /  ${simd_len}]), "
                  << (i * stride + src_idx) % simd_len << "));";
            if (is_big_oc) {
                fw_ss << "c[1][" << i
                      << "] = "
                         "GiFloat16Type2FixLenType(GiSimdFmaLaneFloat16("
                         "GiFixLenType2GiFloat16Type(c[1]["
                      << i << "]), GiFixLenType2GiFloat16Type(weight[1][" << weight_idx
                      << "]),  GiFixLenType2GiFloat16Type(src[(" << i
                      << " * ${stride} + " << src_idx << ") / ${simd_len}]), "
                      << (i * stride + src_idx) % simd_len << "));";
            }
        }
    }
...
std::string render_store(
    ...
    for (int ow_idx = 0; ow_idx < nr_ow; ++ow_idx) {
        ss << act.GenIntrinsicFloatStore(
                "GiFixLenType2GiFloat16Type(c[" + std::to_string(c_idx) + "][" +
...
std::string render_kernel(TContext* ctx) {
    std::stringstream ss;
    ss << R"(typedef void kern_func (const gi_float16_t* src_ptr, const gi_float16_t* filter_ptr, const gi_float16_t* bias_ptr,
                                        gi_float16_t* dst_ptr, const int packed_iw, const int packed_ic_stride, const int ld_dst_oc);   
    )";

    auto mode = ctx->getAttrStr("nonlineMode");
    auto activate_gen = create_activation_gener_instrinsic(mode, "f16");
    ...
    kernel_render.add_ctx_int("pad_h")
            ...
            .add("render_core",
                 [=]() {
                     return render_core(
                             src_reg_size, filter_size, true, ld_weight_fw,
                             ld_weight_oc, simd_len, ow_step, stride, false);
                 })
            .add("render_store",
    ...
    static inline void nchw_nchw88_s${stride}_${filter_size}x${filter_size}_kernel_${nr_ow}_oc${oc_step}(const gi_float16_t* src_ptr, const gi_float16_t* filter_ptr, const gi_float16_t* bias_ptr,
                                            gi_float16_t* dst_ptr, const int packed_iw, const int packed_ic_stride, const int ld_dst_oc){
            const int pad_h = ${pad_h};
            const int pad_w = ${pad_w};
            const int simd_len = ${simd_len};
            GI_FLOAT16_FIXLEN_t c[${oc_step}/8][${ow_step}];
            ${render_init(0)}
            ${render_init(1)}

            for (int ic_idx = 0; ic_idx < ${ic}; ++ic_idx) {
                GI_FLOAT16_FIXLEN_t src[${src_reg_size}];
                GI_FLOAT16_FIXLEN_t weight[${oc_step}/8][${filter_size}];                
                ${render_core()}
    ...
    static inline void
    nchw_nchw88_s${stride}_${filter_size}x${filter_size}_kernel_remain_oc${oc_step}(const
    gi_float16_t* src_ptr, const gi_float16_t* filter_ptr, const gi_float16_t* bias_ptr,
                                            gi_float16_t* dst_ptr, const int
                                            packed_iw, const int
            ...
            ${activate_init()}
            rep(step, dst_nr_ow){
                ${activate(GiFixLenType2GiFloat16Type(c[0][step]), dst_ptr + step * simd_len)};
            }
            rep(step, dst_nr_ow){
                ${activate(GiFixLenType2GiFloat16Type(c[1][step]), dst_ptr + ld_dst_oc + step * simd_len)};
            }
        }
    )";
    ...
    static inline void
    nchw_nchw88_s${stride}_${filter_size}x${filter_size}_kernel_${nr_ow}_oc${oc_step}(const
    gi_float16_t* src_ptr, const gi_float16_t* filter_ptr, const gi_float16_t* bias_ptr,
                                        gi_float16_t* dst_ptr, const int packed_iw,
...
std::string ConvFloat16NCHWNCHW88::GetKernelBody(TContext* ctx) const {
    std::stringstream writer;
    auto src_tensor = ctx->getAttrOprand("operand:0");
    uint32_t ic = src_tensor.shape[1];
    auto dst_tensor = ctx->getAttrOprand(
...
static inline void copy_pad_src(gi_float16_t* sptr_dst, const gi_float16_t* sptr_origin,
    ...
    rep(ic_idx, ic) {
        const gi_float16_t* sptr = sptr_origin + ic_idx * ic_stride;
        memset(sptr_dst, 0, sizeof(gi_float16_t) * packed_iw * pad_top);
        ...
        rep(ih_idx, ih) {
            ...
            memcpy(sptr_dst, sptr, sizeof(gi_float16_t) * iw);
            sptr_dst += iw;
            sptr += iw;
        ...
        memset(sptr_dst, 0, sizeof(gi_float16_t) * packed_iw * pad_bottom);
    ...
    const gi_float16_t* input_ptr = inputs[0]->ptr;
    const gi_float16_t* weight_ptr = inputs[1]->ptr;
    const gi_float16_t* bias_ptr = inputs[2]->ptr;
    gi_float16_t* output_ptr = outputs[0]->ptr;
    ...
    const int pack_oc_size = 8;
    const int big_oc_step = ${big_oc_step};
    const int oh_step = 1;
    ...
    gi_float16_t* workspace_ptr = workspace->ptr;
    ...
    const gi_float16_t* src_ptr = workspace_ptr;
    for (int batch_idx = 0; batch_idx < batch_size; ++batch_idx){
        ...
        if (oc_remain){
            const int weight_offset = oc_end * ic * ${filter_size} * ${filter_size};
            for (int oh_idx = 0; oh_idx < oh; oh_idx += oh_step) {
                for (int ow_idx = 0; ow_idx < ow_end; ow_idx += ow_step) {
                    const int src_offset =  oh_idx * stride_h * packed_iw +
                                            ow_idx * stride_w;
                    const int dst_offset = oc_end * oc_stride +
                                            (oh_idx * ow + ow_idx) * pack_oc_size;
                    ${oc_remain_kern}(src_ptr + src_offset, weight_ptr + weight_offset, bias_ptr + oc_end, output_ptr + dst_offset, packed_iw, packed_ic_stride, ld_dst_oc);
                }
                if (ow_remain) {                    
                    const int src_offset =  oh_idx * stride_h * packed_iw +
                                            ow_end * stride_w;
                    const int dst_offset = oc_end * oc_stride +
                                            (oh_idx * ow + ow_end) * pack_oc_size;
                    ${oc_remain_kern_tail}(src_ptr + src_offset, weight_ptr + weight_offset, bias_ptr + oc_end, output_ptr + dst_offset, packed_iw, packed_ic_stride, ld_dst_oc, ow_remain);
                }
            }
        }
    ...
    std::string main_kern =
            "nchw_nchw88_s" + std::to_string(stride) + "_" +
            std::to_string(filter_size) + "x" + std::to_string(filter_size) +
            "_kernel_" + std::to_string(ow_step) + "_oc" + std::to_string(big_oc_step);
    std::string main_kern_remain = "nchw_nchw88_s" + std::to_string(stride) + "_" +
                                   std::to_string(filter_size) + "x" +
                                   std::to_string(filter_size) + "_kernel_remain_oc" +
                                   std::to_string(big_oc_step);
    std::string oc_remain_kern =
            "nchw_nchw88_s" + std::to_string(stride) + "_" +
            std::to_string(filter_size) + "x" + std::to_string(filter_size) +
            "_kernel_" + std::to_string(ow_step) + "_oc" + std::to_string(pack_oc_size);
    std::string oc_remain_kern_tail =
            "nchw_nchw88_s" + std::to_string(stride) + "_" +
            std::to_string(filter_size) + "x" + std::to_string(filter_size) +
            "_kernel_remain_oc" + std::to_string(pack_oc_size);
    writer << StringTemplate::StringTemplateArgs(ctx)
                      ...
                      .add("oc_remain_kern", oc_remain_kern.c_str())
                      .add("oc_remain_kern_tail", oc_remain_kern_tail.c_str())
                      .render(body_temp);
    return writer.str();
}
