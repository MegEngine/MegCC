/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/Fp32ChannelWiseMk4.cpp
 *
...
struct ComputPadBorder {
    ...
    ComputPadBorder(int kernel, int stride, int pad_h, int pad_w,
                    std::string nonline_mode, bool flt_GI)
            : m_kernel(kernel),
              m_pad_h(pad_h),
              m_pad_w(pad_w),
              m_stride(stride),
...
static std::string compute_vec(std::vector<std::string> str) {
    ...
    for (int i = 0; i < compute_size; ++i) {
        ss << dst << " = GiFloat32Type2FixLenType(GiMlaqFloat32(GiFixLenType2GiFloat32Type(" << dst << "), GiFixLenType2GiFloat32Type((" << src1 << ")[" << i
...
static std::string load_vec_x(std::vector<std::string> str) {
    ...
    for (int i = 0; i < times; i++) {
        ss << "(" << dst << ")[" << i << "] = GiFloat32Type2FixLenType(GiLoadFloat32((" << src
           << ")+ 4 *" << i << "));\n";
    }
    return ss.str();
}
