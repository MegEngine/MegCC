/**
 * \file
 * compiler/lib/KernelGen/GeneralIntrinsic/WarpAffine.cpp
 *
...
static short global_table[ (1 << INTER_BITS) *  (1 << INTER_BITS) * 2 * 2];
...
static inline void remapBilinear_c2(const uint8_t* src, uint8_t* dst, int swidth,
                                 int sheight, int sstep, int dwidth,
                                 int dheight, int dstride, const short* xy,
                                 int xy_stride, const unsigned short* fxy,
                                 int fxy_stride, const short* wtab, const uint8_t* bval) {
...
static inline void remap(const uint8_t* src_ptr, uint8_t* dst_ptr, int ih,
    ...
    for (int y1 = 0; y1 < brows0; ++y1) {
        ...
        GI_INT16_t v_scale = GiBroadcastInt16(INTER_TAB_SIZE2 - 1);
        int x1 = 0;
        for (; x1 <= bcols0 - 8; x1 += 8) {
            GiStoreInt16((int16_t*)A + x1, GiAndInt16(GiLoadInt16((int16_t*)sA + x1), v_scale));
        }
        for (; x1 < bcols0; ++x1)
    ...
    if(ch == 1){
        ...
        remapBilinear_c2(src_ptr, dst_ptr, iw, ih, iw * ch, bw, bh, ow * ch,
                    xy + y * bw * 2 + x * 2, bw * 2, bufa, bw, wtab, bval);
    }else if(ch==3){
...
static inline void warpaffine_img(const uint8_t* src_ptr, int ic, int ih, int iw, uint8_t* dst_ptr, int oh ,int ow, const double* M, const short* ctab, const uint8_t* board_val){
    ...
    for (int task_id = 0; task_id < all_task; ++task_id) {
        ...
        for (int y1 = 0; y1 < bh; ++y1) {
            ...
            short* alpha = A + y1 * bw;
            GI_INT32_t v__X0 = GiBroadcastInt32(X0), v__Y0 = GiBroadcastInt32(Y0),
                    v_mask = GiBroadcastInt32(INTER_TAB_SIZE - 1);
            int x1 = 0;
            for (; x1 + 8 <= bw; x1 += 8) {
                GI_INT32_t v_X0 = GiShiftRightInt32(
                        GiAddInt32(v__X0, GiLoadInt32(adelta + x + x1)),
                        AB_BITS - INTER_BITS);
                GI_INT32_t v_Y0 = GiShiftRightInt32(
                        GiAddInt32(v__Y0, GiLoadInt32(bdelta + x + x1)),
                        AB_BITS - INTER_BITS);
                GI_INT32_t v_X1 = GiShiftRightInt32(
                        GiAddInt32(v__X0, GiLoadInt32(adelta + x + x1 + 4)),
                        AB_BITS - INTER_BITS);
                GI_INT32_t v_Y1 = GiShiftRightInt32(
                        GiAddInt32(v__Y0, GiLoadInt32(bdelta + x + x1 + 4)),
                        AB_BITS - INTER_BITS);
                GI_INT16_t v_xy0, v_xy1;
                v_xy0 =
                        GiCombineInt16Low(GiCvtInt32ToInt16(GiShiftRightInt32(v_X0, INTER_BITS)),
                                    GiCvtInt32ToInt16(GiShiftRightInt32(v_X1, INTER_BITS)));
                v_xy1 =
                        GiCombineInt16Low(GiCvtInt32ToInt16(GiShiftRightInt32(v_Y0, INTER_BITS)),
                                    GiCvtInt32ToInt16(GiShiftRightInt32(v_Y1, INTER_BITS)));

                GiStoreInt16(xy + (x1 << 1), GiZipV0Int16(v_xy0,v_xy1));
                GiStoreInt16(xy + (x1 << 1) + 8, GiZipV1Int16(v_xy0, v_xy1));

                GI_INT16_t v_alpha0 = GiCvtInt32ToInt16(GiAddInt32(
                        GiShiftLeftInt32(GiAndInt32(v_Y0, v_mask), INTER_BITS),
                        GiAndInt32(v_X0, v_mask)));
                GI_INT16_t v_alpha1 = GiCvtInt32ToInt16(GiAddInt32(
                        GiShiftLeftInt32(GiAndInt32(v_Y1, v_mask), INTER_BITS),
                        GiAndInt32(v_X1, v_mask)));
                GiStoreInt16(alpha + x1, GiCombineInt16Low(v_alpha0, v_alpha1));
            }
...
std::vector<KernelObj> WarpAffineKernel::GetDependInternalSymbol(
        TContext* ctx) const {
    ...
    if (is_nhwc_u8) {
        return {{remap_table.GetKernelSymbol(ctx),
                 remap_table.GetKernelBody(ctx),
                 remap_table.GetBodyGuardBegin(ctx),
                 remap_table.GetBodyGuardEnd(ctx),
    ...
    std::string normal_temp_body = R"(
        rep(batch_idx, batch){
            ...
            rep(ow_idx, ow){
                ...
                rep(ic_idx, ic){
                    float val = visit_src(batch_src_ptr, ic_idx, ih0, iw0, sstrd) * (1.0f-alphaw) * (1-alphah) +
                                visit_src(batch_src_ptr, ic_idx, ih0, iw1, sstrd) * alphaw * (1.0f-alphah) +
                                visit_src(batch_src_ptr, ic_idx, ih1, iw0, sstrd) * (1.0f-alphaw) * alphah +
