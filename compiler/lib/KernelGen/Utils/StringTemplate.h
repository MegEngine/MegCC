...
public:
    ...
    static std::function<R(Args...)> object_bind(R (C::*func)(Args...) const,
                                                 C& instance) {
    ...
    public:
        StringTemplateArgs(TContext* ctx = nullptr) : m_ctx(ctx) {
            init_builtin();
        };
        ...
        StringTemplateArgs& add(const std::string& key,
        ...
        std::string try_get_str(const std::string& key) {
            return m_kv_map[key];
        }
