/**
 * \file
 * compiler/lib/Dialect/Kernel/Transforms/migrate/static_mem_alloc/pushdown.cpp
 *
...
size_t safe_sub(size_t a, size_t b, size_t min_delta = 0) {
    CC_ASSERT(a >= b + min_delta);
    return a - b;
}
static int freeblock_cls_count = 0;
}  // namespace
    ...
    // find prev allocated interval
    {
        ...
        if (!m_mem_block.empty() && p != m_mem_block.begin()) {
            ...
            if (p != m_mem_block.begin() && !p->allocated()) {
                --p;
                CC_ASSERT(p->allocated()) << "found adjacent free blocks";
            }
            if (p->allocated())
                iprev = p->interval;
        }
...
PREALLOC_MEM(void)::free(AllocResult& alloc_rst) {
    CC_ASSERT(alloc_rst.m_valid);
    auto iter = alloc_rst.m_iter;
    CC_ASSERT(iter->allocated());
...
size_t StaticMemAllocPushdown::get_interval_addr_end(Interval* interval) {
    ...
    for (auto i = ow_root; i; i = i->overwrite_src()) {
        CC_ASSERT(i == ow_root || i->overwrite_dest_root() == ow_root);
    ...
    CC_ASSERT(interval->addr_begin != INVALID);
