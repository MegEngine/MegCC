/**
 * \file
 * compiler/lib/Dialect/Kernel/Transforms/migrate/static_mem_alloc/interval_move.cpp
 *
    ...
    for (size_t cur_idx = 0; cur_idx < m_interval.size(); ++cur_idx) {
        ...
        for (auto i : conflict) {
            ...
            if (i->addr_begin < cur->addr_begin) {
                ...
                CC_ASSERT(i->addr_end() <= cur->addr_begin);
            } else {
                m_interval_extra[cur->id].move_conflict.push_back(i);
                CC_ASSERT(cur->addr_end() <= i->addr_begin);
            }
        }
    }
...
void StaticMemAllocIntervalMove::insert_interval(
    ...
    for (auto i : conflict) {
        ...
        CC_ASSERT(!i->addr_overlap(dest));
    }
    CC_ASSERT(m_peak == orig_peak + peak_incr);
}
...
std::pair<size_t, size_t> StaticMemAllocIntervalMove::find_best_fit(
    ...
    CC_ASSERT(best_fit_addr != INVALID);
...
void StaticMemAllocIntervalMove::move_interval_higher(Interval* interval,
    ...
    for (auto i : m_interval_extra[interval->id].move_conflict) {
        ...
        CC_ASSERT(!interval->addr_overlap(*i));
    }
}
