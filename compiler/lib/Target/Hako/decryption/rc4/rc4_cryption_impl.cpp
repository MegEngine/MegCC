/**
 * \file compiler/lib/Target/Hako/decryption/rc4/rc4_cryption_impl.cpp
 *
...
std::vector<uint8_t> NaiveEncrypImpl::decrypt_model() {
    std::vector<uint8_t> result(m_model_length, 0);
    uint8_t* ptr = result.data();
    size_t enc_byte_len = (m_model_length / 8 - 1) / 10 * 8;
    uint8_t* enc_key_8 = (uint8_t*)&m_enc_key;
    for (size_t i = 0; i < enc_byte_len; ++i) {
        ptr[i] = static_cast<const uint8_t*>(m_model_mem)[i];
        ptr[i] ^= enc_key_8[i % 8];
    }
    for (size_t i = enc_byte_len; i < m_model_length; ++i) {
        ptr[i] = static_cast<const uint8_t*>(m_model_mem)[i];
    }
    return result;
}

std::vector<uint8_t> NaiveEncrypImpl::encrypt_model() {
    return {};
}
