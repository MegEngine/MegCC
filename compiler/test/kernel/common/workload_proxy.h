/**
 * \file
 * compiler/test/kernel/common/workload_proxy.h
 *
...
struct WorkloadOprProxy {
    ...
    static KernelWorkload get_workload(Opr* opr, const TensorNDArray& tensors) {
        KernelWorkload res;
        res.compute_workload_go =
                (float)get_compute_workload(opr, tensors) / 1e9;
        res.memory_workload_mb = 0;
        for (size_t i = 0; i < tensors.size(); ++i) {
            res.memory_workload_mb += tensors[i].layout.total_nr_elems() *
                                      tensors[i].layout.dtype.size();
        }
