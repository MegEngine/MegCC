...
class BaseConfig():
    ...
    def __init__(self):
        ...
        if redis_url:
            self.result_cache = RedisResultCache(redis_url,
                                                 datetime.timedelta(weeks=1))


class Test():

    def __init__(self, config_file: str, name: str, id_hash: str,
    ...
    def run_local(self, step: dict):
        ...
        subprocess.run(args='&&'.join(step_cmd),
                       shell=True,
                       env=build_envs,
    ...
    def run_remote(self, step_name: str, remote_workdir: str, step: dict):
        ...
        assert device_tags, 'device tags in step {} of target {} cannot be empty '.format(
            step_name, self.__name)
        if self.__use_bundle and not self.__bundle_id:
            bundle_client = bundle.BundleClient(
                address=base_config.korok_server,
                token=base_config.korok_token,
                group=base_config.korok_group)
            bundle_id = bundle_client.create_bundle(tags=device_tags,
        ...
        if oss_enabled and input_files:
            ...
            oss_prepare_cmd = [
                ...
                'aws configure set aws_access_key_id {}'.format(
                    base_config.oss_access_key_id),
                'aws configure set aws_secret_access_key {}'.format(
                    base_config.oss_secret_access_key),
                'aws --endpoint-url={} s3 cp --recursive s3://{}/ {}/'.format(
                    base_config.oss_internal_host,
                    os.path.join(base_config.oss_bucket, self.__id_hash,
                                 self.__name, step_name), remote_workdir),
            ]
            cmd = '&&'.join([cmd] + oss_prepare_cmd)
            for file in input_files:
                self.__s3_client.upload_file(
                    file, base_config.oss_bucket,
                    os.path.join(self.__id_hash, self.__name, step_name,
        ...
        cmd = '&&'.join([cmd, 'cd {}'.format(remote_workdir)] +
                        step.get('cmd'))
        ...
        if oss_enabled and output_files:
            for file_name in output_files:
                cmd = '&&'.join([
                    cmd, 'aws --endpoint-url={} s3 cp {} s3://{}'.format(
                        base_config.oss_internal_host, file_name,
                        os.path.join(base_config.oss_bucket, self.__id_hash,
                                     self.__name, step_name, file_name))
                ])
        elif not oss_enabled and output_files:
            download_files.append((file_name, './'))
        taskClient = task.TaskClient(address=base_config.korok_server,
                                     token=base_config.korok_token,
                                     group=base_config.korok_group)
        taskClient.create_task_with_log(tags=device_tags,
                                        command=[cmd],
            ...
            for step_name in self.__target:
                ...
                if cache_files and base_config.result_cache:
                    metadata = [
                        ...
                        step.get('cmd'),
                        step.get('envs'),
                        step.get('device-tags')
                    ]
                    cache_hash_key = self.compute_cache_key(
                ...
                if type == 'remote':
                    remote_workdir = '-'.join(
                        ['ci', self.__id_hash, self.__name, step_name])
                    self.run_remote(step_name, remote_workdir, step)
                else:
                    self.run_local(step)
                end_time = time.time()
                print('{} duration: {}s'.format(step_name,
                                                end_time - start_time))
...
def main():
    ...
    for case_name in args.tests:
        testConfig = Test("./compiler/script/ci/remote_test_config.yaml",
                          case_name, hash_id, base_config)
        testConfig.run()
