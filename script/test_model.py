...
def compare_file(file_path_0, file_path_1, eps):
    ...
    print(d0.shape)
    print('max diff ', np.max(diff.flatten()), 'abs_diff',
          np.max(np.abs(d0 - d1).flatten()), d0[max_idx:max_idx + 10], " vs ",
          d1[max_idx:max_idx + 10], ' at ', max_idx)
...
def auto_check(model_name_2_all, eps, target_arch, target_host, env, mdl_str):
    ...
    if not os.path.exists(tinynn_test_lite_path):
        subprocess.run([
            build_script, "--cross_build", "--remove_old_build",
            "--kernel_dir", kern_dir, "--cross_build_target_arch", target_arch,
            "--specify_build_dir", build_dir
        ])
    ...
    for model_file in os.listdir(model_dir):
        if model_file.endswith('.tiny'):
            ...
            input_list, model_data_info, model_shape_info, model_data_info_local = parse_model_info(
                model_infos, input_dir)
            for input_info in input_list:
                if not os.path.exists(input_info['input_path']):
                    gen_input(input_info['input_path'], input_info['shape'],
                              input_info['dtype'])
            ...
            target_call(target_host, [
                'cd', target_run_dir, '&&', './tinynn_test_lite', model_file,
                'tiny_out', '0', "'" + model_data_info + "'", target_shape_info
            ])
            ...
            if model_name in model_name_2_all:
                ...
                local_call([
                    mgb_runner_path, mdl_model_path, mgb_out_dir,
